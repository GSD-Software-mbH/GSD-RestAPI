...
class _RestApiExampleState extends State<RestApiExample> {
  ...
  void _initializeApiManager(RestApiDOCUframeManager restApiDOCUframeManager) {
    ...
    // Subscribe to events
    apiManager.onSessionIdChanged = ((e) async {
      setState(() {
        _sessionId = apiManager.sessionId;
        _addLog('Session ID changed: $_sessionId');
      });
    });

    apiManager.onUserAndPassWrong = ((e) async {
      setState(() {
        _addLog('Authentication failed: ${e.message}');
      });
    });

    apiManager.onLicenseWrong = ((e) async {
      setState(() {
        _addLog('License error detected: ${e.message}');
      });
    });

    apiManager.onLogMessage = ((e) async {
      setState(() {
        _addLog('RestApiDOCUframeManager: $e');
      });
    });
