...
class RestApiDOCUframeManager {
  ...
  RestApiDOCUframeManager(
    ...
    firebase_performance.FirebasePerformance? firebasePerformance,
  ...
  Future<http.Response> _http(
    ...
    do {
      ...
      onLogMessage?.call("Request Body: ${(body?.length ?? 0) > 1000 ? "${body?.substring(0, 1000)}..." : body}");
      ...
      if (useRequestHeader && requestHeader != null) {
        try {
          ...
          RefreshSessionResponse response = await _refreshSession(twoFaToken: newToken);
  ...
  Future<http.Response> _performRequest({
    ...
    final String requestHash =
        '$requestUri|${requestHeader.toString()}|${body.toString()}'
            .toMd5Hash();
      ...
      try {
        ...
        firebase_performance.HttpMetric? metric = !kIsWeb
            ? performance?.newHttpMetric(
                "https://gsd-software.com/$function",
                firebase_performance.HttpMethod.Post,
              )
            : null;
        await metric?.start();
    ...
    _pendingResponses[requestHash] = RestApiRequest(
      requestFunction,
      login: login,
    );
  ...
  /// }
  /// ```
  Future<RefreshSessionResponse> _refreshSession({int retryCount = 3, String twoFaToken = ""}) async {
    bool active = false;
    ...
    for (i = 1; i < retryCount; i++) {
      RestApiLoginResponse response = await login(_password, twoFactorAuthToken: twoFaToken, refreshSession: true);
  ...
  Future<RestApiResponse> uploadFile(
    ...
    try {
      ...
      await _postUploadFile(pathToFile, uploadID, webFile: webFile, name: name);
  ...
  /// Lädt eine Datei mit einem Upload-Controller hoch
  ///
  /// Überträgt eine lokale Datei oder Web-Datei auf den Server mit einem
  /// Controller für asynchrone Überwachung des Upload-Status.
  ///
  /// [pathToFile] - Pfad zur lokalen Datei (für Desktop/Mobile)
  /// [webFile] - PlatformFile für Web-Uploads (optional)
  /// [replaceOID] - OID eines bestehenden Dokuments zum Ersetzen
  ...
  /// [fetchToObject] - Dokument-Objekt nach Upload erstellen (Standard: true)
  ///
  /// Returns: [RestAPIFileUploadController] zur Überwachung des Upload-Fortschritts
  ///
  /// Der Upload-Controller ermöglicht:
  /// - Asynchrone Überwachung des Upload-Status
  /// - Fehlerbehandlung während des Uploads
  /// - Callback-Funktionen für Erfolg/Fehler
  ///
  /// Beispiel:
  /// ```dart
  ...
  Future<RestAPIFileUploadController> uploadFileWithController(
    ...
    try {
      // Upload-ID vom Server anfordern
      RestApiResponse restApiResponse = await _getUploadID();
      ...
      // Controller erstellen und zu aktiven Uploads hinzufügen
      RestAPIFileUploadController controller = RestAPIFileUploadController(
        uploadID,
      );
      _activeUploads[uploadID] = controller;

      // Asynchronen Upload-Prozess starten
      _postUploadFile(pathToFile, uploadID, webFile: webFile, name: name).then(
        (restApiResponse) async {
          ...
          // Optional: Dokument-Objekt in der Datenbank erstellen
          if (fetchToObject) {
            try {
  ...
  Future<RestApiResponse> _postUploadFile(
    ...
    try {
      ...
      if (webFile == null) {
        request.files.add(await http.MultipartFile.fromPath('', pathToFile, filename: name));
      } else {
        request.files.add(
          http.MultipartFile.fromBytes(
            "",
            webFile.bytes ?? [],
            filename: name ?? webFile.name,
          ),
        );
      }
  ...
  /// }
  /// ```
  Future<RestApiResponse> validate2FASecret(String appname, String token) async {
    try {
  ...
  /// }
  /// ```
  Future<RestApi2FASecretResponse> get2FASecret(String appname, String username) async {
    try {
