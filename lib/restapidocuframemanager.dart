...
class RestApiDOCUframeManager {
  ...
  Future<http.Response> _performBufferedRequest(
    ...
    if (_requestBuffer.length >= _config.maxBufferSize) {
      _flushRequestBuffer();
    }
  ...
  Future<RestApiLoginResponse> login(
    ...
    try {
      ...
      String uriPath =
          "${_config.alias.isEmpty ? "" : "/${_config.alias}"}/${_config.v2Login ? v2Loginfunction : v2LoginSecurefunction}";
      Uri requestUri = _getUri(uriPath);
      ...
      final response = RestApiLoginResponse(
        await _http(
          HttpMethod.post,
          requestUri,
          _config.v2Login ? v2Loginfunction : v2LoginSecurefunction,
          handleSession: false,
          body: bodyText,
          requestHeader: requestHeader,
          decryptRSA: !_config.v2Login,
          decryptAES: _config.v2Login,
          login: true,
  ...
  /// Throws: HttpRequestException bei HTTP-Fehlern
  /// Throws: SessionInvalidException bei ungültiger Session
  Future<RestApiResponse> getSyncObjectsOfClass(
    String appName,
    String className,
  ) async {
    try {
