...
class RestApiDOCUframeManager {
  ...
  bool get allowSslError => _allowSslError;

  /// Getter für die Anzahl der Elemente pro Seite
  int get perPageCount => _perPageCount;

  /// Getter für Multi-Request-Modus
  bool get multiRequest => _multiRequest;

  /// Getter für maximale Buffer-Größe
  ...
  /// Aktiviert Multi-Request-Modus für Batch-Verarbeitung
  /// Verfügbar in GSD-Connect ab Version 1.0.0.20
  ///
  /// Bei true werden mehrere schnell aufeinanderfolgende API-Anfragen
  /// zusammengefasst und als eine einzige Batch-Anfrage an den Server
  /// gesendet. Dies kann die Performance bei vielen gleichzeitigen
  /// Operationen erheblich verbessern.
  ///
  /// Vorteile:
  /// - Reduzierte Netzwerk-Latenz durch weniger HTTP-Requests
  /// - Verbesserte Server-Performance durch Batch-Verarbeitung
  /// - Geringere Ressourcennutzung bei Massen-Operationen
  ///
  /// Nachteile:
  /// - Leicht erhöhte Komplexität bei der Fehlerbehandlung
  /// - Potentiell höhere Memory-Nutzung durch Request-Pufferung
  ///
  /// Standard: false für maximale Kompatibilität
  bool _multiRequest = false;

  /// Buffer für Multi-Request-Verarbeitung
  ...
  late int _bufferFlushDelayMs;

  /// Aktuelle Multi-Request-Verarbeitung läuft
  bool _isProcessingMultiRequest = false;
  ...
  /// ```dart
  /// manager.onMissing2FAToken = () async {
  ...
  /// Beispiel-Implementierung:
  /// ```dart
  /// manager.onLogMessage = (String message) async {
  ...
  /// Beispiel-Implementierung:
  /// ```dart
  /// manager.onLicenseWrong = (LicenseException e) async {
  ///   showDialog(
  ///     context: context,
  ...
  /// Beispiel-Implementierung:
  /// ```dart
  /// manager.onSessionIdChanged = (String sessionId) async {
  ...
  /// Beispiel-Implementierung:
  /// ```dart
  /// manager.onUserAndPassWrong = (UserAndPassWrongException e) async {
  ...
  /// Callback-Funktion für HTTP-Metrik-Aufzeichnung
  ///
  /// Diese optionale Callback-Funktion wird nach jedem HTTP-Request aufgerufen
  /// und übergibt detaillierte Performance-Metriken für Monitoring und Analyse.
  ///
  /// Die Funktion erhält ein RestApiHttpMetric-Objekt mit folgenden Informationen:
  /// - Request-Methode (GET, POST, PUT, etc.)
  /// - Funktionsname des API-Endpunkts
  /// - HTTP-Status-Code der Antwort
  /// - Antwort-Größe in Bytes
  /// - Content-Type der Antwort
  /// - Start- und End-Zeitstempel
  /// - Berechnete Request-Dauer
  ///
  /// Parameter: [metric] - Das RestApiHttpMetric-Objekt mit Performance-Daten
  ///
  /// Beispiel-Implementierung:
  /// ```dart
  /// manager.onHttpMetricRecorded = (RestApiHttpMetric metric) async {
  ///   print("API Call: ${metric.httpMethod} ${metric.requestName}");
  ///   print("Duration: ${metric.duration}ms");
  ///   print("Response: ${metric.responseCode} (${metric.responsePayloadSize} bytes)");
  ///
  ///   // An externes Monitoring-System senden
  ///   await analyticsService.recordApiMetric(metric);
  /// };
  /// ```
  ...
  /// [bufferFlushDelayMs] - Verzögerung für das Puffern von Anfragen (Standard: 100ms)
  /// [maxBufferSize] - Maximale Anzahl gepufferter Anfragen vor automatischem Flush (Standard: 10)
  /// [sessionid] - Bereits vorhandene Session-ID (optional)
  /// [allowSslError] - SSL-Fehler ignorieren (Standard: false)
  /// [multiRequest] - Mehrfache Anfragen mit einem Request abarbeiten (Standard: false, Verfügbar ab GSD-Connect 1.0.0.30)
  /// [onMissing2FAToken] - Callback-Funktion für fehlende 2FA-Token (optional)
  RestApiDOCUframeManager(
    ...
    _multiRequest = multiRequest;
    _maxBufferSize = maxBufferSize;

    _baseUri = Uri.parse(serverUrl);

    _client = createClient(_connectionTimeout, allowSslError: _allowSslError);
  }
  ...
  /// Setzt den Multi-Request-Modus
  ///
  /// [multiRequest] - true für Multi-Request-Modus, false für Einzelanfragen
  void setMutliRequest(bool multiRequest) {
    _multiRequest = multiRequest;
  }

  /// Setzt die maximale Buffer-Größe für Multi-Request-Verarbeitung
  ///
  /// [maxBufferSize] - Die maximale Anzahl an Anfragen, die gepuffert werden sollen
  ...
  Future<http.Response> _http(
    ...
    do {
      ...
      onLogMessage?.call("RequestUri send: $requestUri");
      onLogMessage?.call("Request Header: $requestHeader");
      onLogMessage?.call(
      ...
      if (!httpResponse.body.isValidJson()) {
        ...
        if (decryptRSA) {
          decodedResponseBody = await EncryptionManager().decryptRSAInBlocks(
            httpResponse.body,
          );
        }
        ...
        if (decryptAES) {
          ...
          Map<String, dynamic> aesJson = {
            ...
            "data": base64Encode(
              bodyPartBytes.sublist(16, bodyPartBytes.length),
            ),
          };
      ...
      if (useRequestHeader && requestHeader != null) {
        try {
          ...
          onUserAndPassWrong?.call(e);
          rethrow;
        } on LicenseException catch (e) {
          onLicenseWrong?.call(e);
          rethrow;
          ...
          if (onMissing2FAToken != null) {
            newToken = await onMissing2FAToken!();
          }
  ...
  Future<http.Response> _performRequest({
    ...
    // Bestimmte Request-Typen werden nie gepuffert
    if (_multiRequest && !_shouldNeverBuffer(function)) {
      return await _performBufferedRequest(
        requestUri,
        requestHeader,
        body,
        method,
    ...
    try {
      ...
      onHttpMetricRecorded?.call(request.metric!);
      _pendingResponses.remove(requestHash);
    }
  }

  /// Verarbeitet eine gepufferte Anfrage
  Future<http.Response> _performBufferedRequest(
    Uri requestUri,
    Map<String, String>? requestHeader,
    dynamic body,
    HttpMethod method,
    ...
    // Request zum Buffer hinzufügen
    _requestBuffer.add(bufferedRequest);

    // Buffer-Flush-Timer starten/neu starten
    _resetBufferFlushTimer();

    // Sofortiger Flush bei Buffer-Überlauf
    if (_requestBuffer.length >= _maxBufferSize) {
      _flushRequestBuffer();
    }
  ...
  /// Setzt den Buffer-Flush-Timer zurück
  void _resetBufferFlushTimer() {
    _bufferFlushTimer?.cancel();

    if (_requestBuffer.isNotEmpty) {
      _bufferFlushTimer = Timer(
        Duration(milliseconds: _bufferFlushDelayMs),
        _flushRequestBuffer,
      );
    }
  }

  /// Leert den Request-Buffer und sendet Multi-Request oder Einzelrequests
  void _flushRequestBuffer() {
    if (_requestBuffer.isEmpty || _isProcessingMultiRequest) return;

    _bufferFlushTimer?.cancel();
    _isProcessingMultiRequest = true;

    // Kopie des Buffers erstellen und originalen Buffer leeren
    List<RestAPIBufferedRequest> requestsToProcess = List.from(_requestBuffer);
    _requestBuffer.clear();

    // Prüfen ob nur ein Request vorhanden ist
    if (requestsToProcess.length == 1) {
      // Einzelanfrage direkt verarbeiten
      _retryAsSingleRequest(requestsToProcess.first)
          .then((_) {
            _isProcessingMultiRequest = false;
      ...
      _processMultiRequest(requestsToProcess)
          .then((_) {
            _isProcessingMultiRequest = false;
  ...
  /// Verarbeitet mehrere Anfragen als Multi-Request
  Future<void> _processMultiRequest(
    ...
    try {
      ...
      // Multi-Request senden
      RestApiHttpMetric metric = RestApiHttpMetric(function, HttpMethod.post);

      try {
        metric.start();

        final multiRequest = http.Request('POST', requestUri);

        Map<String, String> headers = _getHeader();
        multiRequest.headers.addAll(headers);
        multiRequest.body = jsonEncode(requestsList);

        onLogMessage?.call("Multi-Request send: $requestUri");
        onLogMessage?.call(
        ...
        onHttpMetricRecorded?.call(metric);
      }
    } catch (error) {
      // Bei Fehler alle wartenden Requests mit Einzelanfragen durchführen
      onLogMessage?.call(
        "Multi-Request failed, falling back to individual requests: $error",
      );
  ...
  Future<void> _retryAsSingleRequest(
    ...
    try {
      RestApiRequest request = RestApiRequest(
        ...
        bufferedRequest.requestHeader,
        bufferedRequest.body,
  ...
  Future<RestApiLoginResponse> login(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/${v2Login ? v2Loginfunction : v2LoginSecurefunction}";
      Uri requestUri = _getUri(uriPath);
      ...
      if (v2Login) {
        ...
        bodyText = await _getv2LoginSecureBody(
          jsonEncode(requestBodyMap),
          serverPublicKey,
        );
      }
      ...
      final response = RestApiLoginResponse(
        await _http(
          HttpMethod.post,
          requestUri,
          v2Login ? v2Loginfunction : v2LoginSecurefunction,
          handleSession: false,
          body: bodyText,
          requestHeader: requestHeader,
          decryptRSA: !v2Login,
      ...
      if (response.isOk) {
        ...
        onSessionIdChanged?.call(_sessionId);
      }
      return (response);
    } catch (e) {
      _sessionId = "";
  ...
  Future<RestApiResponse> getFolderByType(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/$function/$folderType";
  ...
  Future<RestApiResponse> getFolderByPath(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/$function/$encodedPath";
  ...
  Future<RestApiResponse> postMailReply(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/$function/$sourceMailOid";
      Map<String, String> params = {};
  ...
  Future<Uint8List?> getPreview(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/$function/$parameters$keepRatioPath$objectOidPath$pagePath";
      Uri requestUri = _getUri(uriPath);
  ...
  Future<RestApiResponse> patchFoldersAdd(
    ...
    try {
      ...
      String uriPath =
          "${alias.isEmpty ? "" : "/$alias"}/$function/$folderTypeEncoded/$folderIdEncoded";
      Map<String, String> params = {};
  ...
  Future<RestApiResponse> _postUploadFile(
    ...
    try {
      ...
      onLogMessage?.call("Uploadfile send: $requestUri");
      onLogMessage?.call("Request Header: $requestHeader");
  ...
  Future<String> _getv2LoginSecureBody(
    String clearBody,
    ...
    // Daten verschlüsseln
    final encryptedBase64 = await EncryptionManager().encryptRSAInBlocks(
      clearBody,
      publicKey: publicKey,
    );
