...
/// `DOCUframeApi` (mit `api.v1`/`api.v2`/`api.raw`) verwenden.
@Deprecated(
  'RestApiDOCUframeManager ist eingefroren und wird durch DOCUframeApi ersetzt. '
  'Neue Aufrufe verwenden DOCUframeApi (api.v1/api.v2/api.raw). '
  'Entfernung frühestens in einem späteren Major-Release.',
)
class RestApiDOCUframeManager {
  ...
  Future<http.Response> _http(
    ...
    do {
      ...
      if (useRequestHeader && requestHeader != null && handleResponse) {
        try {
  ...
  /// };
  ///
  /// http.Response response = await manager.postExecuteInterfaceMacro(
  ///   "MeinCustomMakro",
  ///   macroData
  /// );
  ///
  ...
  Future<http.Response> postExecuteInterfaceMacro(
    ...
    String function = "v1/execute/$macroName";
    String uriPath =
        "${_config.alias.isEmpty ? "" : "/${_config.alias}"}/$function";
    Uri requestUri = _getUri(uriPath);
  ...
  /// Beispiel:
  /// ```dart
  /// RestApiResponse response = await manager.deleteFolders("/Temp/Alter_Ordner", notEmpty: true);
  /// if (response.isOk) {
  ///   print("Ordner erfolgreich gelöscht");
  /// }
  /// ```
  Future<RestApiResponse> deleteFolders(
    String path, {
    bool notEmpty = false,
  }) async {
    try {
      ...
      if (notEmpty) params['notEmpty'] = notEmpty.toString();
  ...
  Future<RestApiResponse> deleteObject(
    ...
    try {
      ...
      if (actions.isNotEmpty) params['actions'] = actions;
      if (moveToRecycler) params['moveToRecycler'] = moveToRecycler.toString();
  ...
  ///   }
  /// ]);
  /// RestApiResponse response = await manager.postAction(
  ///   "GSDObjekt",
  ...
  /// Beispiel:
  /// ```dart
  /// RestApiResponse response = await manager.getDocumentPaths("doc-12345", extended: true);
  /// if (response.isOk) {
  ...
  Future<RestApiResponse> getDocumentPaths(
    ...
    try {
      ...
      Uri requestUri = _getUri(
        uriPath,
        params: {"extended": extended.toString()},
      );
  ...
  ///
  /// Returns: Response mit der Server-Antwort
  ///
  ...
  Future<http.Response> customRequest(
    ...
    try {
      uriPath =
          "${_config.alias.isEmpty ? "" : "/${_config.alias}"}${uriPath.startsWith("/") ? "" : "/"}$uriPath";
