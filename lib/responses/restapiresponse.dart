  ...
  RestApiResponse(this._httpResponse) {
    ...
    if (!responseJson.containsKey("status")) {
      throw HttpRequestException(
        ...
        httpResponse.statusCode as String,
      ...
      if (!statusJson.containsKey("internalStatus")) {
        throw FormatException(
          "missing 'status.internalStatus' field in response body",
          responseJson,
        );
      } else {
      ...
      if (!statusJson.containsKey("statusMessage")) {
        throw FormatException(
          "missing 'status.statusMessage' field in response body",
          responseJson,
        );
      } else {
    ...
    if (_internalStatus == "0" || _internalStatus == "200") {
      _isOk = true;
    } else if (_internalStatus == "201") {
      throw SessionInvalidException(
        "webservice error: $_internalStatus $_statusMessage",
        _internalStatus,
        _statusMessage,
      );
    } else if (_internalStatus == "204") {
      throw TokenOrSessionIsMissingException(
        "webservice error: $_internalStatus $_statusMessage",
        _internalStatus,
        _statusMessage,
      );
    } else if (_internalStatus == "302") {
      throw UserAndPassWrongException(
        "webservice error: $_internalStatus $_statusMessage",
        _internalStatus,
        _statusMessage,
      );
    ...
    } else if (_internalStatus == "340") {
      throw Require2FALoginException(
        "webservice error: $_internalStatus $_statusMessage",
        _internalStatus,
        _statusMessage,
      );
    } else if (_internalStatus == "341") {
      throw Missing2FATokenException(
        "webservice error: $_internalStatus $_statusMessage",
        _internalStatus,
        _statusMessage,
      );
