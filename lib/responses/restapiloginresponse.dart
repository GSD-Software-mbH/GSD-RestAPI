...
class RestApiLoginResponse extends RestApiResponse {
  ...
  List<RestApiACLEntry> applications = [];
      ...
      if (!responseJson.containsKey("data")) {
        ...
        if (!dataJson.containsKey("sessionId")) {
          throw const FormatException(
            "missing 'data.sessionId' field in response body",
          );
        } else {
        ...
        dynamic aclElements = acls["~Elements"];

        isMdm2 = true;

        for (var i = 0; i < aclElements.length; i++) {
          currentAclApplication = aclElements[i];

          if (currentAclApplication != null) {
            applications.add(RestApiACLEntry.fromJson(currentAclApplication));
          }
        }
      }
    }
  ...
  bool hasApplication(String application) {
    return applications.any((acl) => acl.application == application);
  }
}
