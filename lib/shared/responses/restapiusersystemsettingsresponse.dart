...
class RestApiUserSystemSettingsResponse extends RestApiResponse {
  ...
  /// Name des Standard-Terminbenutzers für den Kalender
  ///
  /// Leer = der eingeloggte Benutzer wird verwendet.
  /// Kann einen Benutzer- oder Benutzergruppen-Namen enthalten.
  String dateUserObjectName = "";

  /// Ereigniseinstellungen für den Benutzer die über Event-Makros konfiguriert sind
  ...
  RestApiUserSystemSettingsResponse(super._httpResponse) {
    ...
    if (isOk) {
      if (!responseJson.containsKey("data")) {
        ...
        noPromptInsertNewDocsUserFolders =
            dataJson['noPromptInsertNewDocsUserFolders'] ?? false;
        noPromptInsertNewDocsGlobalFolders =
            dataJson['noPromptInsertNewDocsGlobalFolders'] ?? false;
        ...
        missedCallsCount = dataJson['missedCallsCount'] ?? 0;
        dateUserObjectName = dataJson['dateUserObjectName'] ?? "";

        if (dataJson.containsKey("eventSettings")) {
          eventSettings = dataJson['eventSettings'];
        }
      }
    }
  }
